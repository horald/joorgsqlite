...
include("../config.php");
  ...
  $verz = dir ($pfad);
  $urlpfad = str_replace($www,$httppfad,$pfad);
  ...
  echo "<a href='bilderschau.php?pfad=".$pfad."&id=".$id."' class='btn btn-primary btn-sm active' role='button'>Bilderschau</a>"; 
  ...
  while ( $entry = $verz->read () )
  {
    if (($entry!=".") AND ($entry!="..")) {
	  ...
	  if (substr($entry,0,7)<"2016-10") {
	  	 $lWeiter=false;
	  }
	  	//echo ">".substr($entry,1,7)."<";
	  ...
	  if ($lWeiter==true) {
        ...
        if ($wildcard==".jpg") {
          echo "<td><img src='".$urlpfad.$entry."' alt='".$entry."' height='50' width='auto'></td>";
        } else {
          echo "<td>.</td>";
        }
        echo "<td><a href='anzeigen.php?pfad=".urlencode($pfad)."&filename=".urlencode($entry)."' target='_blank' class='btn btn-primary btn-sm active' role='button'>Anzeigen</a></td>";
        echo "<td><a href='notiz.php?filename=".$entry."' class='btn btn-primary btn-sm active' role='button'>Notiz</a></td>";
